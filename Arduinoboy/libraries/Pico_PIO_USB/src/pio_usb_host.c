...
#ifndef PIO_USB_DISCONNECT_DEBOUNCE_US
// The stock check treats roughly 2 us of SE0 as a disconnect. With a hardware
// hub on a software-timed host, short SE0/line-state glitches can tear down the
// root port and surface as random 0/0 enumeration. Require SE0 to remain stable
// before declaring a disconnect.
#define PIO_USB_DISCONNECT_DEBOUNCE_US 50
#endif

#ifndef PIO_USB_BULK_IN_ACCEPT_ANY_DATA_PID
// Work around DATA0/DATA1 desync on bulk IN. The lower layer has already ACKed
// a CRC-good DATA packet by the time we inspect the PID. If we discard that
// packet without aligning our expected DATA PID, a hub/device can keep the MIDI
// IN endpoint logically mounted but silent.
// Enabled by default (was 0, i.e. off): this exact desync is the closest known
// match for "USB MIDI host input stops receiving permanently, DIN keeps working,
// only a reset brings it back" - a stuck toggle mismatch means every future
// packet from the device arrives on the PID the host no longer expects, gets
// silently discarded forever, with nothing left to naturally resync it. See
// arpnmidi/legacy/USB2514B_PIO_USB_HOST_HANDOFF.txt item 9/21 for the original
// investigation. Left as an overridable default (#ifndef), not hardwired, so a
// build can still opt back out with -DPIO_USB_BULK_IN_ACCEPT_ANY_DATA_PID=0.
#define PIO_USB_BULK_IN_ACCEPT_ANY_DATA_PID 1
#endif
...
static bool __no_inline_not_in_flash_func(se0_stays_asserted)(
    root_port_t *port, uint32_t debounce_us) {
  uint32_t start = get_time_us_32();
  while ((uint32_t)(get_time_us_32() - start) < debounce_us) {
...
static bool __no_inline_not_in_flash_func(connection_check)(root_port_t *port) {
  if (pio_usb_bus_get_line_state(port) == PORT_PIN_SE0) {
    if (!se0_stays_asserted(port, PIO_USB_DISCONNECT_DEBOUNCE_US)) {
      return true;
    }
...
static int __no_inline_not_in_flash_func(usb_in_transaction)(pio_port_t *pp,
  ...
  if (receive_len >= 0) {
    bool const is_data_pid = receive_pid == USB_PID_DATA0 || receive_pid == USB_PID_DATA1;
    bool const accept_bulk_in_data_pid = is_data_pid &&
                                         ((ep->attr & 0x03) == EP_ATTR_BULK) &&
                                         (ep->ep_num & EP_IN) &&
                                         PIO_USB_BULK_IN_ACCEPT_ANY_DATA_PID;
    if (receive_pid == expect_pid || accept_bulk_in_data_pid) {
      if (accept_bulk_in_data_pid && receive_pid != expect_pid) {
    ...
    if ((ep->attr & 0x03) == EP_ATTR_BULK && (ep->ep_num & EP_IN)) {
      // Treat transient bulk-IN decode/no-response misses like NAKs. MIDI devices
      // are often idle, and surfacing a brief PIO receive miss as XFER_FAILED can
      // leave the class driver mounted but silent.
      ep->failed_count = 0;
    } else if (++ep->failed_count >= TRANSACTION_MAX_RETRY) {
