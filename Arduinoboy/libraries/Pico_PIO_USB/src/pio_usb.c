...
static inline __force_inline bool pio_usb_bus_wait_for_rx_start(const pio_port_t* pp) {
  ...
  // Full-speed devices behind an external hub can start their response later
  // than a directly connected device. Use extra margin so random hub/repeater
  // turnaround jitter does not look like a missing response to the PIO host.
  // Low speed keeps the existing 12 us window.
  uint32_t start = get_time_us_32();
  uint32_t timeout = pp->low_speed ? 12 : 8;
